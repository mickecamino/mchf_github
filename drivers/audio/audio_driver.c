...
*/
	...
	if(!dir)	{	// Conversion is "above" on RX (LO needs to be set lower)
		...
		arm_mult_f32(ads.q_buffer, ads.Osc_I_buffer, ads.b_buffer, size/2);
		//
		arm_mult_f32(ads.q_buffer, ads.Osc_Q_buffer, ads.c_buffer, size/2);	// multiply products for converted Q channel
		arm_mult_f32(ads.i_buffer, ads.Osc_I_buffer, ads.d_buffer, size/2);
		//
		...
		arm_mult_f32(ads.q_buffer, ads.Osc_Q_buffer, ads.a_buffer, size/2);	// multiply products for converted I channel
		arm_mult_f32(ads.i_buffer, ads.Osc_I_buffer, ads.b_buffer, size/2);
		//
		arm_mult_f32(ads.i_buffer, ads.Osc_Q_buffer, ads.c_buffer, size/2);	// multiply products for converted Q channel
		arm_mult_f32(ads.q_buffer, ads.Osc_I_buffer, ads.d_buffer, size/2);
		//
...
static void audio_rx_processor(int16_t *src, int16_t *dst, int16_t size)
{
	...
	//
	arm_scale_f32(ads.i_buffer, (float32_t)ts.rx_adj_gain_var_i, ads.i_buffer, size/2);
	//
	arm_scale_f32(ads.q_buffer, (float32_t)ts.rx_adj_gain_var_q, ads.q_buffer, size/2);
	//
	//
	...
	switch(ts.dmod_mode)	{
		...
		case DEMOD_AM:
			...
			break;
		default:	// do USB demod as default if we end up here somehow!
			arm_add_f32(ads.i_buffer, ads.q_buffer, ads.a_buffer, size/2);	// sum of I and Q
			break;
	}
	//
	...
	if((!ads.af_dissabled) && (ts.dsp_active & 4) && (ts.dmod_mode != DEMOD_CW) && (!ts.dsp_inhibit))	{	// No notch in CW mode
		...
		//
		arm_lms_norm_f32(&lms2Norm_instance, ads.a_buffer, &lms2_nr_delay[lms2_outbuf], errsig2, ads.a_buffer, psize/2);	// do automatic notch
		//
	...
	if((ts.dsp_active & 1) && (!(ts.dsp_active & 2)) && (!ads.af_dissabled) && (!ts.dsp_inhibit))	{	// Do this if enabled and "Pre-AGC" DSP NR enabled
		...
		//
		arm_lms_norm_f32(&lms1Norm_instance, ads.a_buffer, &lms1_nr_delay[lms1_outbuf], ads.a_buffer, errsig1 ,psize/2);	// do noise reduction
		//
	...
	//
	// AGC function - Look-ahead type (KA7OEI)
	//
	for(i = 0; i < psize/2; i++)	{
...
//
//*----------------------------------------------------------------------------
//* Function Name       : audio_tx_compressor (look-ahead type)
//* Object              :
...
static void audio_tx_compressor(int16_t size, float gain_scaling)
{
	...
	//
	arm_mult_f32(ads.i_buffer, ads.agc_valbuf, ads.i_buffer, size/2);		// Apply ALC gain corrections to TX audio channels
...
static void audio_tx_processor(int16_t *src, int16_t *dst, int16_t size)
{
	...
	{
		softdds_runf(ads.i_buffer,ads.q_buffer,size/2);		// generate tone/modulation for TUNE
		//
	...
	//
		...
		// Equalize based on band and simultaneously apply I/Q gain adjustments
		//
		arm_scale_f32(ads.i_buffer, (float32_t)(ts.tx_power_factor * ts.tx_adj_gain_var_i * SSB_GAIN_COMP), ads.i_buffer, size/2);
		arm_scale_f32(ads.q_buffer, (float32_t)(ts.tx_power_factor * ts.tx_adj_gain_var_q * SSB_GAIN_COMP), ads.q_buffer, size/2);
		//
		// ------------------------
	...
	//
		...
		if(ts.iq_freq_mode)	{				// is translation active?
			...
			//
			// Now, generate the upper sideband of the AM signal:  We must do much of this again to produce an identical (but "different") signal.
			//
			for(i = 0; i < size/2; i++)	{				// restore data from temporary holding buffer, swapping I/Q to get USB
